                    ...
                    <Collapsible.Root>
                        ...
                        <Collapsible.Content right="5" position="absolute" flex="auto" zIndex={999}>
                            <Box padding="3" borderWidth="1px" bgColor="white" color="black" borderRadius="md">
                                {permissoes === "Admin" && (
                                    <Link className="text-blue-500 flex flex-row items-center gap-3 pb-4" href={"/configuracoes"}><FiSettings /> Configurações </Link>
                                )}
                                <Link className="text-red-500 flex flex-row items-center gap-3" href={"/login"} onClick={() => {
                                    localStorage.removeItem("unialfa.token");
                                    localStorage.removeItem("unialfa.nome");
                                    localStorage.removeItem("unialfa.permissoes");
                                }}><FaSignOutAlt /> Desconectar</Link>
                            </Box>
                        </Collapsible.Content>
                    </Collapsible.Root>
                </div>
